...
<head>
    ...
    <style type="text/css">
        ...
        .search-popup-wrapper input {
            padding-right: 80px;
            /* chừa chỗ cho mic + icon search */
        }

        .search-popup-mic {
            position: absolute;
            right: 48px;
            /* nằm trước icon search */
            top: 50%;
...
<body>
    ...
    <div class="search-popup position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center p-6 bg-black bg-opacity-75">
        <div class="search-popup__form position-relative">
            ...
            <form action="{{ route('shop_all') }}" method="GET" class="search-popup-form">
                <div class="search-popup-wrapper position-relative">

                    <input
                        class="search-popup__field border-0 border-bottom bg-transparent text-white w-100 py-3"
                        name="query"
                        type="text"
                        placeholder="Search…"
                    ...
                    <!-- 🎤 Voice Search -->
                    <i class="fa fa-microphone search-popup-mic" id="voice-search-popup"></i>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function bindVoiceSearch(micSelector) {
                document.querySelectorAll(micSelector).forEach(mic => {
                    mic.addEventListener('click', () => {
                        currentMic = mic;
                        currentInput = mic.closest('form').querySelector('input[name="query"]');
                        currentForm = mic.closest('form');
                        recognition.start();
                    });
                });
            }

            // 🎤 bind cho sidebar + popup
            bindVoiceSearch('#voice-search');
            bindVoiceSearch('#voice-search-popup');

            recognition.onstart = function() {
                if (currentMic) currentMic.style.color = 'red';
            };

            recognition.onresult = function(event) {
                const text = event.results[event.results.length - 1][0].transcript;
                if (currentInput) currentInput.value = text.trim();


            };

            recognition.onend = function() {
                if (currentMic) currentMic.style.color = '';
            };
